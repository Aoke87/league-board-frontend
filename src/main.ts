...
(async function () {
    ...
    const matchesResponse = await fetch(`${import.meta.env.VITE_BACKEND_URL}match-v5/latest`);
    const initialMatches = await matchesResponse.json();
    const summonerResponse = await fetch(`${import.meta.env.VITE_BACKEND_URL}summoner/all`);
    ...
    store.commit('setBackendUrl', import.meta.env.VITE_BACKEND_URL);
